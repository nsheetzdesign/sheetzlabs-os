...
export async function action({ request, context }) {
    ...
    const to_emails = formData.get('to_emails');
    const cc_emails = formData.get('cc_emails');
    const bcc_emails = formData.get('bcc_emails');
    ...
    const { data: draft } = await supabase
        ...
        .insert({
        account_id,
        to_emails: to_emails ? to_emails.split(',').map((s) => s.trim()).filter(Boolean) : [],
        cc_emails: cc_emails ? cc_emails.split(',').map((s) => s.trim()).filter(Boolean) : [],
        subject,
        body_text: body,

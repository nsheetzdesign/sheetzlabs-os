...
export function InboxSidebar({ accounts, counts, globalCounts, activeFolder, activeAccountId, activeLabel, onSelectFolder, onSelectLabel, onCompose, onDragOver, onDrop, }) {
    ...
    const submitNewLabel = (accountId) => {
        const name = newLabelName.trim();
        if (!name)
            return;
        fetcher.submit({ account_id: accountId, name }, { method: 'post', action: '/dashboard/inbox/labels' });
        setNewLabelName('');
                        ...
                        return (_jsxs("div", { className: "mb-1", children: [_jsxs("button", { onClick: () => toggleAccount(account.id), className: `flex items-center gap-2 w-full px-2 py-1.5 text-left rounded group ${isExpanded ? 'bg-zinc-800/50' : 'hover:bg-zinc-800/30'}`, children: [isExpanded
                                            ...
                                            return (_jsxs("button", { onClick: () => onSelectFolder(label.name.toLowerCase(), account.id), onDragOver: (e) => handleDragOver(e, label.name, 'folder', account.id), onDragLeave: handleDragLeave, onDrop: (e) => handleDrop(e, label.name, 'folder', account.id), className: `flex items-center gap-2 w-full px-2 py-1 rounded text-xs transition-colors ${isActive
                                                    ...
                                                    : isDragOver
                                                        ...
                                                        }), creatingLabelFor === account.id ? (_jsxs("div", { className: "flex items-center gap-1 px-2 py-1", children: [_jsx("input", { autoFocus: true, value: newLabelName, onChange: (e) => setNewLabelName(e.target.value), onKeyDown: (e) => {
                                                                        if (e.key === 'Enter')
                                                                            submitNewLabel(account.id);
                                                                        if (e.key === 'Escape') {
                                                                            setCreatingLabelFor(null);
                                                                            setNewLabelName('');
                                                                        }
                                                                    }, placeholder: "Label name", className: "flex-1 bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-xs text-zinc-200 focus:outline-none focus:border-emerald-500" }), _jsx("button", { onClick: () => submitNewLabel(account.id), disabled: !newLabelName.trim() || fetcher.state !== 'idle', className: "text-emerald-400 hover:text-emerald-300 disabled:opacity-40", title: "Create", children: _jsx(Check, { size: 14 }) })] })) : (_jsxs("button", { onClick: () => { setCreatingLabelFor(account.id); setNewLabelName(''); }, className: "flex items-center gap-2 w-full px-2 py-1 text-xs text-zinc-500 hover:text-zinc-300", children: [_jsx(Plus, { size: 12 }), "Create label"] })), fetcher.data?.error && creatingLabelFor === null && (_jsx("p", { className: "px-2 py-1 text-xs text-red-400", children: fetcher.data.error }))] }))] }))] }))] }, account.id));

...
export function EmailHtmlFrame({ html }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (cancelled)
                return;
            setSrcDoc(`<!DOCTYPE html><html><head><meta charset="utf-8">` +
    ...
    const resize = () => {
        ...
        if (!win)
            return;
        try {
            const next = Math.max(win.document.body?.scrollHeight ?? 0, win.document.documentElement?.scrollHeight ?? 0);
            if (next)
                setHeight(next);
        }
        catch {
    ...
    if (!mounted) {
        return _jsx("div", { className: "text-sm text-zinc-500 italic", children: "Loading message\u2026" });
    }
    return (_jsx("iframe", { ref: iframeRef, title: "Email content", sandbox: "allow-same-origin allow-popups allow-popups-to-escape-sandbox", referrerPolicy: "no-referrer", srcDoc: srcDoc, onLoad: handleLoad, style: { width: "100%", height, border: "none", display: "block" } }));
}

...
## Addendum — 2026-06-12 (Prompt 65A): dispatch gate closed — AG-A1 / AG-A2 RESOLVED

**Status: AG-A2 RESOLVED, AG-A3 contained, AG-A1 RESOLVED (defense-in-depth).** All changes in `apps/api/src/lib/agent-engine.ts`; regression suite `apps/api/src/lib/agent-engine.test.ts` (18 tests, green). No migration — `agent.output_actions` already holds the correct minimal per-agent action sets, so Part 1 is enforced against the existing column.

**Gate 1 — per-agent action allowlist (default-deny).** `executeActions` still gates on `agent.output_actions`, now backed by a code-side `KNOWN_ACTIONS` registry: a granted-but-unknown action type is **rejected and recorded** on the run (`agent_actions` row `rejected:<type>`), not silently ignored. Unknown/empty allowlist ⇒ no actions. meeting-prep (`create_knowledge`,`create_task`) and email-drafter (`create_draft`,`update_relationship`) confirmed already-minimal — not widened.

**Gate 2 — payload column allowlist (kills model mass-assignment).** New `ACTION_COLUMNS` registry defines, per write action, the exact columns the model may set + types + required fields. `buildSafePayload(actionType, modelOutput)` filters to those columns (drops everything else — so `id`, `agent_run_id`, `agent_id`, `status`, `slug`, `external_id` can never be model-set), coerces/validates types, and **rejects** (records, doesn't silently drop) an action with a missing required field or a type-invalid value. Server-controlled columns are set by the engine: `pipeline_id`/`agent_run_id`/`web_research` (evaluation), `slug`+`source_type:'agent'` (knowledge), `agent_run_id`+`account_id`/`reply_to_email_id` from trusted userInput (draft), `external_id`+`is_time_block` (calendar). The old `.insert(payload)` raw-passthrough is gone on every path.

**Gate 3 — IDOR-by-model killed.** `update_relationship`/`update_task` no longer trust `payload.id`. `resolveScopedId` constrains the target to ids in **trusted run scope**: a row loaded into the run's `context` (server-issued query), a row created earlier in the same run, or an id handed in by the triggering code (`userInput.relationship_id`/`task_id`). A model-originated arbitrary id ⇒ rejected + recorded. (Email-drafter's `{{relationship_id}}` flows from trusted userInput, so its legitimate path is unaffected.)

**AG-A3 — external-publish default-denied (belt-and-suspenders).** `post_linkedin`/`queue_linkedin` are in a `PUBLISH_ACTIONS` set: even if a stray `PATCH` grants one to an agent, dispatch can now only ever write a `content_queue` row with **server-forced `status:'draft'`** — never an auto-post, never a cron-postable `'scheduled'` row, never a direct `postToLinkedIn` call from model output. The only path to publish remains the human-gated review UI (`dashboard.agents.content` → authed `POST …/post`). **Safe-enablement path:** to ever auto-publish, add an explicit approval queue with an immutable human step (e.g. an `approved_by`/`approved_at` gate on `postScheduledContent`); do NOT grant a raw publish action to an agent. (The cron still auto-posts pre-existing `'scheduled'` rows — but dispatch can no longer create one, so the injection→post chain is broken.)

**AG-A1 — trust boundary on external content (defense-in-depth).** `buildPrompt` no longer interpolates untrusted content verbatim. Object/array context dumps (emails, calendar_events, knowledge, relationships, …) and free-form external scalars (`event_description`, `attendees`, email body/subject/snippet, etc.) are wrapped in `-----BEGIN/END UNTRUSTED DATA (field: …)-----` markers; `neutralizeFences` redacts any forged marker in the content; a `UNTRUSTED_DATA_PREAMBLE` is prepended to every agent's system prompt framing marker content as DATA never instructions. Residual (documented): short identifier scalars used inline in action-output templates (`event_title`, `idea_name`, `event_location`) are left unwrapped to preserve template fidelity — their content is contained by Gates 1–3 regardless, and `event_title` also appears inside the wrapped `calendar_events` dump.

**Still open (tracked, not in 65A):** AG-B1 loop bound, AG-B2 cost cap, AG-B3 run-overlap/stuck-recovery, AG-B4 `learning.ts` parse guards, AG-C1 `PATCH /agents/:slug` field allowlist, AG-C5 multi-tenant `user_id`. AG-C1 in particular remains the config-side mass-assignment that can flip `output_actions`/`model`/`schedule` — the publish default-deny above blunts its worst payload but it should still be allowlisted.

---

## Addendum — 2026-06-13 (Prompt 65B): execution & cost safety — AG-B1/B2/B3/B4 + AG-C1 RESOLVED

**Status: AG-C1, AG-B1, AG-B2, AG-B3, AG-B4 all RESOLVED.** Only AG-C5 (platform-wide multi-tenant `user_id`) remains, tracked separately. Migration **055_agent_runtime_safety.sql** (expand-only) applied to prod via the session pooler; both workers deployed; full `pnpm e2e` green; `apps/api` vitest **32 tests** (was 18 — +14 for 65B). Changes in `apps/api/src/lib/agent-engine.ts`, `routes/agents.ts`, `routes/learning.ts`, `scheduled.ts`, and the web agents view.

**AG-C1 — config-PATCH locked (Part 0, the regression lock on the 65A gate).** `PATCH /agents/:slug` no longer does `update(body)`. `sanitizeAgentPatch` enforces a strict field allowlist: only **`name`, `description`, `enabled`, `schedule`, `max_tokens`** are settable. `output_actions`, `slug`, `system_prompt`, `user_prompt_template`, `input_sources`, `model`, `department`, `id`, `running_until`, `created_at` are FORBIDDEN — present in the body ⇒ **403** (all-or-nothing; a forbidden field rejects the whole request, never silently strips-and-applies the benign part). Unlisted benign-unknown keys are dropped (default-deny). This closes the "one PATCH away from `post_linkedin`" path the 65A publish default-deny only blunted. (The web UI toggles `enabled` via direct Supabase, not this endpoint, so nothing breaks.) Changing an agent's power set is now a deliberate seed-migration / privileged-operator change, never a routine config PATCH.

**AG-B1 — agentic loop bounded.** `executeAgentWithTools`' `while(true)` now stops on `MAX_ITERATIONS` (default **10**), a wall-clock deadline (default **90s**), or the per-run token budget. On any cap it sets `terminated_reason` and returns a graceful partial — **and skips action dispatch** (a capped run didn't finish reasoning, so its output is unsafe to turn into DB writes).

**AG-B2 — token + cost caps.** Per-run cumulative input+output token ceiling (default **200k**) trips `terminated_reason: token_budget`. Per-day spend ceiling (default **$20 = 2000¢**, summed from `agent_performance.total_cost` for `CURRENT_DATE`): when reached, both executors mark a run `skipped`/`daily_budget`, and the cron skips the whole due-batch with one `__system__` skip marker (no per-agent skip-row spam). All limits are env-configurable (`AGENT_MAX_ITERATIONS`, `AGENT_WALL_CLOCK_MS`, `AGENT_RUN_TOKEN_BUDGET`, `AGENT_DAILY_COST_CAP_CENTS`, `AGENT_RUN_TIMEOUT_SECONDS`) via `resolveAgentLimits` — no magic numbers.

**AG-B3 — run-overlap claim + stuck recovery + per-agent isolation.** New `agents.running_until` lease + `claim_agent(id, timeout)` RPC (atomic conditional `UPDATE … WHERE running_until IS NULL OR running_until < now() RETURNING id` — avoids the PostgREST `.or()` pitfall): both executors claim before running and **release in `finally`**; a still-claimed agent is skipped (`already_running`). `claim_agent` fails **open** on a transport error (availability; the token/cost caps still bound a doubled run). `resetStuckRuns` (cron) resets runs left `running` past the lease to `failed`/`stuck_timeout` (mirrors the email `processScheduledSends` crash-recovery). The cron agent loop is now per-agent `try/catch` so one launch failure never blocks the rest.

**AG-B4 — `learning.ts` (+ `performWebSearch`) guarded.** All four `learning.ts` Anthropic handlers route through `anthropicText()` — checks `res.ok`, guards the JSON parse, validates the content shape, returns a typed result; failures log + return **502** instead of throwing a TypeError into an opaque 500. The curriculum `JSON.parse` is now try/caught. `performWebSearch` gained `.ok` checks + guarded parse on both Serper and Brave paths.

**Part 5 — observability + dead code wired.** The previously-**dead** `agent_performance` table is now written after every run via `record_agent_performance()` (atomic upsert of today's per-agent rollup with running averages; terminated/partial runs count as not-success). It doubles as the daily-spend source. The dashboard agents view now surfaces: **today's spend vs. the daily cap** (red + "agents paused" when over), each department's latest-run terminated/skip reason as a badge, and a per-agent failed/skipped indicator with the reason in a tooltip.

**Tests (Part 6).** `agent-engine.test.ts` adds: config-PATCH allowlist (forbidden-field 403s, benign-set pass, type checks), forced-loop → `max_iterations`, forced-overspend → `token_budget`, run-overlap → `already_running` (model never called), daily-cap → `daily_budget` (model never called), `claimAgent` win/lose/fail-open, `isDailyBudgetExceeded` sum-vs-cap, `resetStuckRuns` filters + payload. A small DI hook (`deps.createMessage`) lets the loop/budget caps run end-to-end against a stubbed Anthropic client.

**Residual:** AG-C5 multi-tenant `user_id` (platform-wide, separate track). The agents module's injection (65A) and execution/cost/config (65B) gaps are now closed.
